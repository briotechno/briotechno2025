...
const ApplyNow = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!amountRegex.test(formData.currentCTC)) {
            newErrors.currentCTC = "Enter a valid number (e.g., 10,000)";
        }
        if (!amountRegex.test(formData.expectedCTC)) {
            newErrors.expectedCTC = "Enter a valid number (e.g., 20,000)";
        }

        const urlRegex = /^(https?:\/\/)?([\w-]+\.)*[\w-]+\.[a-z]{2,}(\/[\w\-._~:/?#[\]@!$&'()*+,;=]*)?$/i;
        if (formData.portfolioLink && !urlRegex.test(formData.portfolioLink)) {
            newErrors.portfolioLink = "Enter a valid URL (e.g., https://www.example.com)";
        }
                    ...
                    {formFields.map(({ label, name, type }) => (
                        ...
                        <label key={name} style={styles.label}>
                            ...
                            {type === "text" && name === "message" ? (
                                ...
                                <>
                                    {type === "number" && ["phoneNumber", "currentCTC", "expectedCTC"].includes(name) ? (
                                        <input
                                            type="text"
                                            inputMode="numeric"
                                            pattern="[0-9]*"
